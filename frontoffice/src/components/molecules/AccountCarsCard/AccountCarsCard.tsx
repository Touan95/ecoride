...
export const AccountCarsCard = ({
  cars,
  onAddCar,
  onEditCar,
  onRemoveCar
}: AccountCarsCardProps) => {

  const List = () => {
    if(cars.length > 0) {
      return cars.map((car)=>{
        return(
          <CarCard key={car.id} {...car} onEditClick={onEditCar} onRemoveClick={onRemoveCar}/>
        )
      })
    }
    return (
      <div className='py-20'>
        <Typography align='center' variant='cardTitle'>Vous n'avez pas encore de véhicules</Typography>
      </div>
    )
  }
  ...
  return (
    <div className={clsxm(['w-full rounded-xl flex flex-col p-5 shadow bg-primary-50 gap-5 items-center'])}>
      <div className='flex relative justify-center w-full'>
        ...
        <Button className='absolute right-0' onClick={onAddCar}>Ajouter</Button>
      </div>
