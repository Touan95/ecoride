...
export type RidePassengerRepositoryInterface = Repository<RidePassengerEntity> & {
  createOrUpdate(ridePassenger: RidePassengerEntityInterface, entityManager?: EntityManager): Promise<RidePassengerEntityInterface>;
...
export const RidePassengerRepository: RidePassengerRepositoryInterface = AppDataSource.getRepository(
  ...
  async createOrUpdate(ridePassenger: UpdateRidePassenger, entityManager?: EntityManager): Promise<RidePassengerEntityInterface> {
  ...
  async getOneByIds(userId: string, rideId: string): Promise<RidePassengerEntityInterface | null>{
    const query = this.createQueryBuilder('ride_passenger')
